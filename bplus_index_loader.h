...
class BPLUSIndexLoader {
  public:
    /** struct for storing inode info on disk
     */
     ...
     */
    typedef struct disk_inode_info {
        std::string i_path;
    ...
    std::vector<index_entry> get_children(std::string path);
  ...
  private:
    ...
    std::string path_;
    std::unordered_map<std::string, BPLUSIndex::header*> file_to_mem_;
